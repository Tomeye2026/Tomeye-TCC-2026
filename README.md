● Nome do projeto:

TomEye

● Nome dos integrantes:

Antonella Aiko Kuwahara,
Diogo Rafael Lopes,
Gabriel de Azevedo Zinetti,
Gabriel Gonzaga Barbosa,
Gabriel Mendonça de Godoy,
Geovana Helena Gonçalves da Cunha,
Heber dos Santos Chinchet,
João Pedro Fogolin Paulelli,


● Curso e turma:

Desenvolvimento de Sistemas, MTEC 3º DS noturno

- Ensino Médio Com Habilitação Profissional De Técnico Em Desenvolvimento De Sistemas


● Professor orientador;

Pedro Ramires da Silva Amalfi Costa


● Breve descrição do projeto (o que ele faz e qual problema pretende resolver);

Sistema Web PWA, voltado ao ramo agrícola da cultura do tomate. Analisando a planta por meio de I.A e buscando possíveis doenças como:
- Septoria;
- Requeima;
- Mofo das folhas;
- Vírus do enrolamento amarelo;
- Mancha bacteriana;

● Tecnologias que serão utilizadas;

- Teachable Machine;
- HTML, JS, CSS;


● Status atual do desenvolvimento;

- Em fase de teste  


● Link para o protótipo (quando disponível);

- https://tom-eye-tcc.netlify.app


● Link para a Wiki do projeto.

- https://github.com/Gzinetti/TomEye/wiki
